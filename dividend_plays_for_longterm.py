...
UNIVERSE = [
    ...
    'KGS',   # Kodiak Gas Services — natural gas compression services (wellhead + gathering + midstream); installs and operates compression units for gas producers under long-term monthly rental contracts = recurring fee revenue regardless of commodity price; GrossM 63.8%, OM 34.2% exceptional (compression asset economics are real); NM 5.8%, ROE 4.6%, D/EV 31.5%; ~3.2% yield; payout 218% GAAP misleading — D&A on compression equipment is massive non-cash charge (same read as EPD/MLP infrastructure, judge by DCF coverage not GAAP payout); FCF 0.2% is the honest flag: growth capex consuming almost all cash as fleet expands; gate: FCF yield crossing 2%+ as fleet utilization matures and capex normalizes; added 2026-08-12
