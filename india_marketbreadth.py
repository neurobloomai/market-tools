"""
India Market Breadth — standalone page
Uses the same UNIVERSE as india_screener.py.
Run: python india_marketbreadth.py
...
from india_screener import UNIVERSE
...
def compute_breadth(tickers):
    ...
    for col in sorted(closes.columns):
        ...
        display = col.replace('.NS', '').replace('.BO', '')
...
def _nh_nl_card(b):
    ...
    if ratio >= 2.0 and nh_trend in ('↑', '→'):
        color = '#3fb950'; status = f'Expanding — NH dominant ({ratio:.1f}×), NIFTY universe has broad tailwind'
    elif ratio <= 0.5 or (nh_trend == '↓' and ratio < 1.0):
        color = '#f85149'; status = f'Deteriorating — NL overtaking ({ratio:.1f}×), raise selectivity on India setups'
    else:
        color = '#e3b341'; status = f'Mixed — NH/NL balanced ({ratio:.1f}×), confirm individual RS vs NIFTY'
...
<div class="card" style="border-color:{color}40">
  <div class="card-title" style="color:{color}">Elder NH/NL Breadth Index — NSE Universe</div>
...
<head>
<meta charset="utf-8">
<title>India Market Breadth — {now}</title>
<style>
...
<body>
<h1>📊 India Market Breadth</h1>
<div class="sub">{now} &nbsp;·&nbsp; {b['total']} names tracked (Quality Growth Universe — NSE)</div>
...
<div style="color:#484f58;font-size:10px;margin-top:16px;border-top:1px solid #21262d;padding-top:8px">
  Data via Yahoo Finance · NSE tickers · For informational purposes only · Not financial advice
</div>
</body>
</html>"""

if __name__ == '__main__':
    print(f"\n  India Market Breadth — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    snap_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'india_breadth_snapshot.json')
