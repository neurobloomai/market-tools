...
if __name__ == '__main__':
    ...
    html     = build_html(data, is_open, status_msg)
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'india_briefing.html')
    ...
    try:
        ...
        commit_msg = f'india_briefing: {now}'
        subprocess.run(['git', 'checkout', '--', 'india_briefing.html'], cwd=repo, capture_output=True)
        subprocess.run(['git', 'pull', '--rebase', 'origin', 'main'],    cwd=repo, check=True, capture_output=True)
        with open(out_path, 'w') as f:
            f.write(html)
        subprocess.run(['git', 'add',    'india_briefing.html'], cwd=repo, check=True, capture_output=True)
