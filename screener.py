...
WATCHLIST = [
    ...
    'PATH',  # UiPath — RPA + AI agent automation layer; ~$1.4B ARR, NRR 115%+; real moat in enterprise process complexity + compliance-heavy industries (finance, healthcare) where auditability creates switching costs; AI pivot is the thesis: RPA bridges legacy systems to AI agents — enterprises can't rip/replace old GUIs, RPA is the interface layer; FCF turned positive; GAAP OM still negative (blocker); Microsoft Power Automate bundling is the structural risk to watch — gate: GAAP OM crossing 0% and sustaining upward trend
