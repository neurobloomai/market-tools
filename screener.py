...
FUTURE_RADAR = {
    ...
    'FSLY': 'Fastly — edge cloud platform (CDN + Compute@Edge + security); differentiated from AKAM on programmability: Compute@Edge uses WebAssembly to run custom logic at the network edge (not just caching) — the right architecture for AI inference at the edge, real-time APIs, IoT, and latency-sensitive apps; Signal Sciences acquisition adds solid WAF/bot management; GrossM 57.1%, FCF 7.7% positive (honest signal — cash generation above GAAP losses), RevG 23.3%, D/EV 0.11 (clean balance sheet); OM -19.0%, NM -19.5%, ROE likely negative — deep blockers; risk: NET (Cloudflare) is converging on the same edge compute territory with stronger developer mindshare + workers platform + 300+ PoPs vs Fastly; Fastly had a major outage in 2021 that damaged reliability trust; LT runway: edge compute is secular as AI inference moves to the network edge + every latency-sensitive application (gaming, IoT, fintech) needs programmable edge; gate to watchlist: OM inflecting toward 0% as Compute@Edge revenue grows as % of mix + FCF expanding past 10% consistently; added FUTURE_RADAR 2026-08-07',
