...
UNIVERSE = [
    ...
    'STX',                        # Seagate Technology — world's largest pure-play HDD maker; nearline (datacenter/cloud) dominant; HAMR (Heat-Assisted Magnetic Recording) = density roadmap moat to 30TB→40TB+ drives enabling premium pricing; AI creates data faster than NAND can store it cheaply (HDD ~10x cheaper per TB) = structural hyperscaler demand; GrossM 45.6% (exceptional for hardware — HAMR ASP premium showing up), OM 43.1%, NM 26.1%, D/EV 0.021, FCF 1.1% (just above gate, near-miss on ≥3% scoring point), RevG 48.5% (AI storage ramp); ROE 371.5% artifact — negative book equity from decades of aggressive buybacks, real ROIC is strong; A+ 7/7; chart correcting hard from extreme AI-storage rally: 3/4 MA (below 10w $891, price $813), 87w extension 160.4% extreme, RSI 51.1 cooling, CMF -0.121 distribution — quality earns UNIVERSE, extension says wait for compression; added 2026-08-08
...
WATCHLIST = [
    ...
    'KMDA',  # Kamada Ltd. — Israeli specialty plasma-derived protein therapeutics; GLASSIA/PROLASTIN-C (Alpha-1 antitrypsin replacement therapy — AAT deficiency is a rare genetic disorder causing progressive lung/liver disease; patients require weekly IV infusions for life = deeply sticky recurring revenue per patient); inhaled AAT program in development (differentiated delivery route vs IV); niche within specialty plasma biologics — not competing in broad IVIG market, owning a specific chronic disease indication; GrossM 41.2% (just above gate), OM 15.4%, NM 11.2%, D/EV 0.033 (near-zero debt), FCF 2.6%, RevG 2.8%; ROE 8.0% only blocker (2% below gate — NI scaling into equity base, not a structural failure; D/EV 0.033 confirms no leverage distortion); 6/7 gates; 2/4 MA (above 10w +0.5% + 87w +2.1%, below 20w/43w), slope falling, RSI 44.8, MACD below signal — chart not yet confirming; RevG 2.8% is the real concern — AAT market is stable chronic disease, not high-growth; geopolitical risk: Israeli company, NASDAQ-listed, regional tensions add volatility layer beyond fundamentals; gate: ROE crossing 10% as NI scales + RevG reaccelerating above 5% as AAT patient additions compound + chart 4/4 recovery; added WATCHLIST 2026-08-10
    'ADMA',  # ADMA Biologics — small-cap plasma-derived immunoglobulin manufacturer; BIVIGAM + ASCENIV (high-titer IGIV for primary immunodeficiency, immune-compromised patients); collects plasma, manufactures specialty plasma proteins end-to-end — vertical integration gives margin control vs pure plasma buyers; turnaround fully executed: real profitability now printing (OM 37.53%, NM 28.80%, ROE 35.56%, GrossM 57.36%, ROA 25.97% per verified source — exceptional for a biologics manufacturer, software-level operating margins); ⚠ yfinance data unreliable for this ticker (shows OM -123% — garbage read from warrant revaluation quarter; D/EV 2.222 also suspect artifact); RevG 15.9% real; grade A on verified metrics but cannot be auto-scored reliably; chart broken: 2/4 MA, -36.7% below 87w — quality intact, structure not confirmed; gate: 43w/87w MA reclaim + yfinance data normalizing so automated screener can grade accurately; added WATCHLIST 2026-08-10
    'DOCS',  # Doximity — physician professional network and clinical communication platform; 80%+ of all US physicians on the platform (not addressable market — actual penetration), making it structurally indispensable; revenue model: (1) pharma/biotech companies pay for physician-targeted drug detailing + CME content to captive, credentialed audience; (2) health system subscriptions for secure clinical messaging (TigerConnect competitor), telehealth tools, and workflow software; GrossM 88.07% (exceptional — pure network economics), FCF 36.56% (cash machine), D/EV 0.0023 (near-zero debt, earns interest income on cash pile — this is why NM 25.48% exceeds OM 21.54%), OM 21.54%, ROE 17.21%, RevG 7.30%; NM computed manually — yfinance netMargins field is null for DOCS but netIncomeToCommon/totalRevenue = 25.48% clean; 7/7 gates A+; chart broken: 2/4 MA, above 10w (+16.0%) + 20w (+15.1%), below 43w (-19.4%) + 87w (-45.5% extreme); stock crashed from $76+ peak to $17.15 low (now $25.63 recovering) — multiple compression as RevG decelerated from 30%+ COVID-era growth to 7.3% as pharma advertising budgets normalized; quality intact, structure broken — fully discounted quality scenario; gate: RevG reaccelerating above 15% as pharma marketing cycles back + 43w/87w MA recovery; added WATCHLIST 2026-08-10
    'PHR',   # Phreesia — patient intake management SaaS for physician offices and health systems; digital intake (replacing clipboards), insurance eligibility verification, scheduling, patient payments, and pharma-sponsored patient education at point-of-care (captive audience at the most receptive moment for health information); two revenue streams: (1) per-patient workflow fees from physician groups/health systems; (2) network solutions = pharma-sponsored content shown to patients during check-in — structurally high-margin because it's digital advertising inventory at clinical scale; GrossM 67.8% (real SaaS economics), D/EV 0.117 (clean balance sheet), FCF 7.3% (positive — honest signal the cash machine is real), RevG 12.9% — four pillars confirmed; OM 5.2%, NM 1.9%, ROE 2.9% below gates but OM approaching 10% (4.8% gap, trending right as S&M spend leverages into installed base); 4/7 gates; chart broken: 2/4 MA, above 10w/20w but below 43w (-6.8%) + 87w (-37.3%) — price $12.44 is deeply discounted from $19+ peak without fundamental deterioration; RSI 53.8, MACD expanding — early recovery signal; gate: OM crossing 10% as S&M leverage compounds into physician network + NM crossing 5% + 43w/87w MA recovery; added WATCHLIST 2026-08-10
    'HAE',   # Haemonetics — blood management technology: NexSys PCS apheresis machines (plasma + platelet collection), BloodTrack hospital blood management software, vascular closure devices; plasma collection is structurally growing — immunoglobulin therapies (IVIG, SCIG) for immune deficiency, neurology, transplant are demand-compounding; HAE machines are contracted with major plasma center operators (CSL, BioLife, Grifols) = recurring disposables revenue on a placed-machine base; GrossM 59.3%, OM 17.9%, NM 7.1%, ROE 11.2%, FCF 7.8%, RevG 5.6%; D/EV 0.237 only blocker — acquisition/restructuring debt; FCF 7.8% is the paydown engine; 6/7 gates; 4/4 MA, RSI 70.0, MACD expanding, 87w ext +31.1% moderate — technically strongest of recent WATCHLIST additions; gate: D/EV below 0.20 via FCF paydown (2-3 quarters at current pace); added WATCHLIST 2026-08-10
    ...
    'EMBC',    # Embecta — BD spinoff (2022), insulin delivery devices (pen needles, syringes, patch pumps); auto-promoted from FUTURE_RADAR 2026-08-07 [grade B, blocking: Debt/EV 0.984]
]
...
FUTURE_RADAR = {
    'SMCI': 'Super Micro Computer — high-density AI GPU server systems integrator; direct liquid cooling architecture + fast customization cycles = Nvidia preferred partner for hyperscaler builds; RevG 122.7% (AI datacenter demand real), ROE 17.9%; GrossM 8.4% (hardware assembler structural ceiling — integrates Nvidia GPUs, doesn\'t make them), OM 6.1%, NM 3.7%, FCF -37% (inventory ramp burn), D/EV 0.344; 2/7 gates; accounting controversy: Hindenburg short report + delayed 10-K filings (FY2024) + near-NASDAQ delisting — audited financials now filed but trust deficit remains; moat debatable: Nvidia also ships through Dell/HPE, SMCI edge is liquid cooling speed-to-market not exclusivity; gate to watchlist: GrossM consistently above 12% + OM crossing 10% + clean audit trail for 2+ consecutive quarters + FCF turning positive; added FUTURE_RADAR 2026-08-08',
    ...
    'DXC':  'DXC Technology — legacy IT services formed from CSC + HP Enterprise Services merger; enterprise IT outsourcing, cloud migration, digital transformation services for Fortune 500; 1/7 gates (only FCF 50.3% passes — likely asset sale proceeds, not operational); OM 2.0%, NM 1.0%, ROE 4.0%, GrossM 23.0% (IT services structural ceiling), RevG -5.1% declining, D/EV 0.980 (debt ≈ entire EV from merger); price $10.92, -23.7% below 87w MA — market pricing distress not recovery; structural problem: legacy outsourcing contracts are low-margin and customers are insourcing or moving to cloud; transformation thesis = AI-enabled managed services + cloud migration as enterprises modernize; gate to watchlist: OM crossing 10% sustained + D/EV below 0.40 via asset sales + RevG turning positive as digital contracts replace legacy; added FUTURE_RADAR 2026-08-07',
    ...
    'STEM': 'Stem, Inc. — AI-driven battery storage software (Athena platform) + hardware; Athena optimizes storage dispatch in real time across grid-scale batteries — routes energy to maximize revenue from grid services, demand charge reduction, and arbitrage; same grid-storage tailwind as FLNC but more distressed balance sheet; GrossM 40.8% (software+hardware blend), OM -37.5%, D/EV 0.97 (debt ≈ entire enterprise value — converts + senior notes, balance sheet near-critical), FCF barely +0.3% (not actively burning but no margin for error), RevG -10.8% declining (hardware project completions lumpy + storage market slowdown); NM 94.2% is one-time debt forgiveness/restructuring, not operational; fwdPE unavailable; price $5.17 (-60% from 200MA) — market pricing survival risk; the Athena software moat is real if the company survives to monetize it (recurring software fees on installed base = toll road); differentiation vs FLNC: more software-native, less project/EPC; risk: D/EV 0.97 with declining revenue = covenant risk if storage market stays weak; gate to watchlist: D/EV below 0.40 (debt restructuring or paydown) + OM inflecting toward 0% + RevG turning positive; realistically 2+ years minimum; added FUTURE_RADAR 2026-07-29',
    'FLNC': 'Fluence Energy — AES + Siemens JV that IPO\'d 2021; grid-scale battery energy storage systems (BESS) + Mosaic software platform for storage optimization; as more intermittent renewables (solar/wind) are added globally, grid-scale storage becomes mandatory infrastructure = structural demand floor; GrossM 11.7% (turnkey project margins — thin by design, software/services mix is the margin lever), OM -8.4%, NM -1.6%, ROE -12.7%, FCF -5.8%, D/EV 0.226 — 5 blockers, all filters failing; RevG 7.7% too slow for the loss-making phase; fwd PE 64x expensive for a loss-making co; price -34% from 200MA; thesis intact but business model still in execution phase — real orders, real projects, not yet profitable; gate to watchlist: GrossM crossing 20%+ as Mosaic software/recurring revenue grows as % of mix + OM turning positive + FCF inflection; 2-3 quarters minimum; added 2026-07-29',
    ...
    'BHC': 'Bausch Health (Canada) — formerly Valeant Pharmaceuticals; operates the Bausch + Lomb eye care franchise (contact lens solutions, Lumify eye drops, surgical products, INFUSE tears) + Salix (GI: Xifaxan, Relistor) + Solta Medical (aesthetic devices); the underlying business is real — GrossM 70.7%, OM 21.3%, FCF 9.7%, RevG 12.7% — but the Valeant acquisition binge left $20.9B in debt on a $2.2B market cap, negative book equity (-$0.6B), D/EV 0.957 (debt ≈ entire EV); NM 1.5% because interest payments consume nearly all operating income; fwdPE 1.4x = market pricing distressed equity, not franchise value; BLCO (Bausch + Lomb) spinoff intended to separate the crown-jewel eye care assets from the pharma+debt complex — execution pending; gate to watchlist: D/EV below 0.50 via sustained FCF deleveraging + NM crossing 5% as interest burden shrinks + BLCO spinoff completing to unlock franchise value; 3-4 quarters minimum; added FUTURE_RADAR 2026-08-07',
    ...
    'SDGR': 'Schrödinger — physics-based computational drug discovery platform (Maestro); quantum mechanical molecular simulation, free energy perturbation, binding affinity prediction, ADMET modeling; differentiated from pure ML drug discovery (which is approximate) — physics-based simulation is accurate but compute-intensive; as compute costs fall, accuracy advantage compounds; used by major pharma (Pfizer, Takeda, Nimbus) + academia as the gold standard for computational chemistry; also runs own drug pipeline on the platform (proprietary oncology + CNS candidates) = software revenue + pipeline optionality; GrossM 56.9%, D/EV 0.103, RevG 7.5% real; OM -70.6%, NM -21.0%, ROE -16.2%, FCF -2.1% — deep investment phase; 3/7 gates; 4/4 MA near 87w base (+2.0%), RSI 64.6, MACD expanding — chart base forming even as fundamentals lag; gate: OM inflecting toward 0% as software ARR compounds + drug pipeline milestones de-risk optionality value; added FUTURE_RADAR 2026-08-10',
    ...
    'CERS': 'Cerus Corporation — INTERCEPT Blood System: pathogen inactivation technology for blood components (platelets, plasma, red blood cells); FDA-approved for platelets + plasma, red blood cell approval pending in US (already approved Europe); blood safety is a permanent regulatory requirement — every transfusion center must use some form of pathogen reduction; GrossM 52.0% (solid), D/EV 0.155 (clean), FCF 0.2% (barely positive), RevG 9.5%; OM -3.2%, NM -3.1%, ROE -10.8% — commercial scaling not yet driving profitability; 4/7 gates; 0/4 daily, 2/4 weekly, weak momentum; gate to watchlist: OM turning positive and sustaining above 10% as INTERCEPT adoption scales across blood centers; added FUTURE_RADAR 2026-08-10',
    'AMWL': 'American Well (Amwell) — telehealth platform; hospital-grade video visit infrastructure for health systems, payers, and employer-sponsored plans; COVID-era beneficiary that discovered the real TAM was smaller than lockdown usage suggested — patients returned in-person, payer reimbursement tightened, in-person preference reasserted; GrossM 52.5% (solid platform economics), D/EV 0.059 (near-zero debt), FCF 6.2% (positive) — three structural pillars intact; critical blockers: RevG -26.6% (revenue actively declining, not just growing slowly — the most serious gate, worse than just being unprofitable), OM -18.4%, NM -35.7%, ROE -29.1%; 3/7 gates; chart misleading: 4/4 MA but at extreme extension — 87w ext +85.9% high, RSI 70.1, stock more than doubled from lows on telehealth revival narrative speculation, not business recovery; this is structure (chart) without thesis (fundamentals) — exactly the case the framework flags; gate: RevG turning positive first (revenue must stop declining before any margin recovery can matter) + OM inflecting toward 0% — both gates must clear before WATCHLIST consideration; added FUTURE_RADAR 2026-08-10',
