...
def parse_watchlist_notes(repo_path):
    """Parse per-ticker thesis notes from inline comments in screener.py WATCHLIST."""
    import re
    from pathlib import Path
    src = (Path(repo_path) / 'screener.py').read_text()
    notes = {}
...
def generate_watchlist_md(score_map, price_map, promos, near_miss, fund_map, repo_path, now):
    """
    Generate WATCHLIST.md — live snapshot of every watchlist name with thesis,
    MA alignment, grade, and blockers. Committed to the repo on every weekly run
    so git history tracks the evolving opportunity case.
    """
    from pathlib import Path

    notes      = parse_watchlist_notes(repo_path)
    promo_set  = {t: (g, ma) for t, _, g, ma in promos}
    nm_map     = {t: (g, ma, bl) for t, _, g, ma, bl in near_miss}

    def grade_for(t):
        if t in promo_set:  return promo_set[t][0]
        if t in nm_map:     return nm_map[t][0]
        d = fund_map.get(t)
        return quality_grade(d) if d else 'ETF'

    def blockers_for(t):
        if t in promo_set:  return 'passes all filters'
        if t in nm_map:     return nm_map[t][2]
        d = fund_map.get(t)
        if d:
            fails = failing_filters(d)
            return ' / '.join(f"{n} {v}" for n, v, _ in fails) if fails else '—'
        return '—'

    def status_for(t):
        ma = score_map.get(t, 0)
        if t in promo_set:  return f'[PROMOTE] {ma}/4 MA'
        if t in nm_map:     return f'[NEAR-MISS] {ma}/4 MA'
        return f'[WATCH] {ma}/4 MA'

    # Sort: MA score desc, then ticker alpha
    ordered = sorted(WATCHLIST, key=lambda t: (-score_map.get(t, 0), t))

    lines = [
        f"# Watchlist — {now}",
        "",
        "Active monitoring list. Each name has a documented thesis and a specific trigger for promotion to UNIVERSE.",
        "Sorted by MA alignment (strongest signal first). Auto-generated on every weekly run.",
        "",
        "| Ticker | Price | MA | Grade | Status | Blockers | Thesis |",
        "|:-------|------:|:--:|:-----:|:------:|:---------|:-------|",
    ]
    ...
    for t in ordered:
        ...
        grade     = grade_for(t)
        status    = status_for(t)
        blockers  = blockers_for(t)
        thesis    = notes.get(t, '—')
        thesis_md = (thesis[:90] + '…') if len(thesis) > 90 else thesis
        lines.append(f"| **{t}** | {price_str} | {ma}/4 | {grade} | {status} | {blockers} | {thesis_md} |")
    ...
    lines += [
        ...
        "",
        "**Status key**",
        "- `[PROMOTE]` — passes all quality filters and MA aligned; auto-promotion pending",
        "- `[NEAR-MISS]` — MA score ≥ 2/4 but quality blockers remain",
        "- `[WATCH]` — below MA threshold; thesis intact, awaiting setup",
        "",
        f"*Auto-generated by `aligned_screener.py` · {now}*",
        "",
        "*Not financial advice. This is a personal framework tracking document.*",
    ]

    md_path = Path(repo_path) / 'WATCHLIST.md'
    md_path.write_text('\n'.join(lines) + '\n')
    return str(md_path)
...
if __name__ == '__main__':
    ...
    generate_watchlist_md(score_map, price_map, promos, near_miss, fund_map, repo_path, now)
    ...
    try:
        ...
        subprocess.run(['git', 'add', 'aligned_screener.html', 'WATCHLIST.md'], cwd=repo, check=True, capture_output=True)
