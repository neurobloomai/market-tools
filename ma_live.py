"""
ma_live.py — Live MA Scanner (currentPrice + iloc[-1])
=======================================================
Companion to ma_scanner.py. Uses live price from yfinance info and
iloc[-1] (latest bar, possibly incomplete intraday) instead of the
confirmed-close iloc[-2] used by ma_scanner.py.

Use this for a real-time read during market hours.
Run: python ma_live.py
...
LIQUID     = ['NVDA', 'META', 'MSFT', 'AAPL', 'AMZN', 'GOOGL', 'AVGO', 'MU', 'NFLX']
BAND_LOW   = -3.0
BAND_HIGH  =  3.0
...
def print_setups(hits, label):
    ...
    if not hits:
        print("  No setups.")
        return
    print(f"  {'Ticker':<7} {'Price':>8}  {'MA':>4}  {'vs MA10d':>9}  {'vs MA10w':>9}  {'Band':>5}")
    for r in sorted(hits, key=lambda x: -x['score']):
        print(f"  {r['ticker']:<7} ${r['price']:>7.2f}  {r['score']}/4  {r['pct10d']:>+8.1f}%  {r['pct10w']:>+8.1f}%  {r['band']}")


if __name__ == '__main__':
    print(f"\nLIVE MA Scanner — {date.today()}  (currentPrice + iloc[-1] MAs)")
    print("="*74)

    with ThreadPoolExecutor(max_workers=9) as ex:
        liquid_rows = list(ex.map(live_scan, LIQUID))
