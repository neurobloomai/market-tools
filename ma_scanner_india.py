"""
ma_scanner_india.py — MA Proximity Scanner  (India / NSE Edition)
==================================================================
...
NSE tickers use the .NS suffix (e.g. RELIANCE.NS, TCS.NS).
India market hours: 9:15 AM – 3:30 PM IST (6.25 hrs/day).
4H is constructed from 1H via positional grouping (every 4 market bars),
consistent with the US version — market-hours aligned, not calendar-based.

Usage
-----
  python3 ma_scanner_india.py                  # scan UNIVERSE
  python3 ma_scanner_india.py --watchlist      # scan WATCHLIST
...
def make_4h(close_1h: pd.Series) -> pd.Series:
    ...
    Calendar resampling misaligns with NSE market hours (9:15–3:30 IST).
    """
...
def print_results(all_results, total, label):
    ...
    print(f"{'Ticker':<16} {'Signal':^9} {'TF':<4} {'Price':>10} {'MA10':>10} {'MA20':>10} {'%MA10':>7}  slope")
    print("─" * 82)
    ...
    for ticker, tfs in sorted_hits:
        ...
        for tf in ['D', '4H', '1H']:
            ...
            prefix = f"{display:<16} {depth:^9}" if first else f"{'':16} {'':9}"
            print(f"{prefix}  {tf:<4} {v['price']:>10.2f} {v['ma10']:>10.2f} {v['ma20']:>10.2f} {v['pct']:>+6.2f}%{arrow}  ↑{v['ma10_slope']:.3f}")
            first = False
        print()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MA Proximity Scanner — India / NSE')
    ...
    print(f"\nMA Scanner — Timeframe Hierarchy  🇮🇳 India / NSE  [{today}]")
