...
UNIVERSE = [
    ...
    'AIAENG.NS',      # AIA Engineering — high-chrome grinding media (balls/cylpebs) for mining/cement mills globally; 130+ countries; consumable replacement model = recurring revenue every 3-6 months per mill; no real competitor at this quality + scale; GrossM 54.8%, OM 26.4%, NM 28.7%, ROE 17.0%, D/EV 0.0% (zero debt), RevG 9.4%; 6/6 gates; A+; added 2026-08-12
    ...
    # Auto & Components + Precision Manufacturing
    'BAJAJ-AUTO.NS','EICHERMOT.NS','FORCEMOT.NS','BHARATFORG.NS','MOTHERSON.NS',
    'BALKRISIND.NS','TIINDIA.NS',
    'SANSERA.NS',   # Sansera Engineering — complex precision forged + machined components (auto + aerospace + non-auto); OM 13.8%, NM 9.3%, GrossM 58.4%, ROE 11.1%, D/EV 0.026 (near-zero debt), RevG 27.8%; FCF -6.0% only blocker — capex expansion into aerospace + non-auto (not distress); A (6/7 structural exception); added 2026-08-11
    'SCHAEFFLER.NS',# Schaeffler India — precision bearings + linear motion components (German parent FAG/LuK/INA); OM 14.8%, NM 11.9%, ROE 21.6%, D/EV 0.001 (zero debt), FCF 3.8%, RevG 17.3%; GrossM 38.4% sole blocker — structural ceiling for precision metal manufacturing (not a software/pharma GrossM story); A (6/7 structural exception); added 2026-08-11
    ...
    'CLEAN.NS',       # Clean Science & Technology — specialty performance chemicals (MEHQ antioxidant, BHA, guaiacol, HALS) used in plastics, food, pharma, cosmetics; proprietary catalytic process = lower cost + higher purity than competition; GrossM 61.5%, OM 28.1%, NM 23.7%, D/EV 0.0% (zero debt), RevG 10.5%; ROE/FCF yfinance gaps (real ROE ~22%); 4/6 gates (gaps only, not real 0s); A; added 2026-08-12
    ...
    'SBIN.NS','CHOLAFIN.NS','MUTHOOTFIN.NS','HDFCAMC.NS','SBIFUNDS.NS',
    ...
    'COLPAL.NS',      # Colgate-Palmolive India — ~55% India toothpaste market share (near-monopoly), ~30% toothbrush share; brand embedded across price tiers from ₹10 sachets to premium whitening; GrossM 68.9% (highest in India consumer), OM 27.7%, NM 21.7%, D/EV 0.1% (zero debt), RevG 11.8%; ROE yfinance gap (real ~55% — asset-light brand business, parent Colgate-Palmolive US shows same pattern); 4/6 gates (ROE+FCF gaps only); A+; added 2026-08-12
    ...
    # Software — BFSI platform
    'OFSS.NS',   # Oracle Financial Services Software — FLEXCUBE core banking platform runs inside 100+ global banks; near-monopoly in Indian banking software; Oracle owns ~73%; OM 59.6%, NM 38.2%, ROE 49.9%, GrossM 58.6%, RevG 68.7%; D/EV 0.000 yfinance artifact (near-zero real debt); A+ on all real metrics; 4/4 MA, RSI 69.1; added 2026-08-08

    # Consumer — beverage franchise
    'VBL.NS',    # Varun Beverages — PepsiCo's largest bottler outside Americas; pan-India geography + Africa/Nepal/Morocco/Zambia international; OM 24.1%, NM 14.0%, ROE 16.9%, D/EV 0.023, FCF 0.1%, GrossM 55.5%, RevG 20.4%; A 7/7; technically broken right now (0/4 MA, RSI 40, MACD falling) — quality earns the UNIVERSE spot, broken chart is the timing signal; watch for weekly MA recovery; added 2026-08-08
]
...
WATCHLIST = [
    ...
    'COCKERILL.NS',   # John Cockerill India (fmr CMI FPE; NSE: COCKERILL, BSE: 500147) — two businesses: (1) steel processing lines (continuous annealing, galvanizing, color coating, acid regeneration for JSW/Tata/ArcelorMittal globally — high-value engineering, project-based); (2) green hydrogen electrolysers (Belgian parent = top-5 global electrolyser maker; India subsidiary manufacturing domestically for India's National Hydrogen Mission); GrossM 55.1% exceptional (product economics real — Belgian IP + engineering value-add); OM 0.5% sole concern (55% gross → near-zero operating = heavy R&D + SG&A compressing margin as electrolyser capacity is built — scale investment phase, not structural failure); NM 4.3% (just below 5% gate), D/EV 0.1% (near-zero debt), RevG 350.8% (large project completion + revenue ramp); ROE/FCF yfinance gaps; P/E 233x (electrolyser optionality priced in); Ramesh Damani accumulated — buying GrossM story + green hydrogen before earnings confirm; 1/6 gates currently (D/EV only) — pre-quality, not yet WATCHLIST standard on OM; gate: OM crossing 8% sustained + NM >5% as electrolyser orders absorb fixed costs; added 2026-08-12
    'HAPPSTMNDS.NS',  # Happiest Minds Technologies — founder-led (Ashok Soota, MindTree co-founder) AI-native IT services; three real P&L segments: IMSS (infra + security), PDES (product/digital engineering), GBS (Generative AI Business Services — dedicated GenAI segment, not AI-washed marketing); digital-first + security-first positioning = structurally differentiated from TCS/Infosys cost-arbitrage model; OM 15.1%, NM 9.3%, ROE 13.3%, RevG 14.3%, EPS G 18.2%, P/E 26.4x (cheap for quality IT); D/EV 31.0% only blocker (unusual for IT services — likely acquisition financing + lease liabilities, not deteriorating balance sheet); FCF yfinance gap; 2/4 MA (-38.6% from 52w high ₹635→₹390, base-building); grade A; gate: D/EV trending below 0.20 + FCF confirming positive; added 2026-08-12
    ...
    'MAXHEALTH.NS',   # Max Healthcare — premium hospital chain (Delhi/NCR, Mumbai, Pune); Apollo's strongest North India competitor; OM 22.7%, NM 17.2%, ROE 14.3%, D/EV 0.033, GrossM 62.4%, RevG 12.2%; 6/7 gates; FCF -0.2% hairline blocker (hospital expansion capex — not structural deterioration); 2/4 MA; gate: FCF consistently positive as new hospital ramp matures; added 2026-08-08
    'CONCORDBIO.NS',  # Concord Biotech — fermentation-based APIs (immunosuppressants: tacrolimus/sirolimus; anti-fungals: amphotericin; complex APIs); listed 2023; moat = fermentation chemistry expertise is hard to replicate, regulatory approvals take years; OM 25.0%, NM 24.8%, GrossM 64.6%, RevG 26.2% — real metrics exceptional; ROE/D/EV/FCF all 0.000 = yfinance data gaps from recent 2023 listing, not actual zeros; 3/4 MA, daily 3/3; gate: yfinance data normalizing on ROE/FCF as filing history builds; added 2026-08-08
    'SATTRIX.BO',     # Sattrix Information Security — pure-play MSSP (managed security services: incident response, VAPT, device mgmt, compliance, detection & response); operates internationally; OM 26.2% exceptional for security services, NM 13.5%, GrossM 44.6%, D/EV 2.2% (near-zero debt), RevG 46.1%, EPS G 44.3%; ROE 6.8% sole blocker (small equity base, fast-growing — ROE should cross 10% as earnings compound into equity); FCF yfinance artifact (SME data — -2.5x revenue is impossible, ignore); BSE SME listed, thin liquidity — extension scan unreliable; Kedia ~8% direct stake; grade B; gate: ROE ≥ 10% + mainboard migration for data reliability; added 2026-08-12
    'CRAFTSMAN.NS',   # Craftsman Automation — precision machined components + aluminum die casting for auto (2W/4W/CV) + industrial; OM 10.8%, NM 5.3%, GrossM 42.8%, D/EV 0.129, RevG 36.3% — all visible gates pass; ROE + FCF missing from yfinance (likely data gap, not operational absence); grade B until confirmed; gate: ROE≥10% + FCF>0 data normalizing; added 2026-08-11
    'TIMKEN.NS',      # Timken India — precision tapered roller bearings + industrial drivetrain components (US parent, 120yr bearing heritage); OM 15.5%, NM 11.8% excellent; GrossM 38.1% structural for precision metal (same read as Schaeffler); ROE + FCF missing from yfinance; grade B until data confirms; gate: ROE/FCF data normalizing + GrossM structural exception review; added 2026-08-11
    'TIIL.NS',        # Triveni Turbine — dominant in industrial steam turbines 0.5-100MW (captive power, sugar/paper/chemical plant WHR, export to 75+ countries); niche with high switching cost (certified for each plant's pressure/temp specs — not interchangeable); GrossM 56.8%, OM 15.0%, NM 10.3%, ROE 15.1%, D/EV 12.7%; FCF -0.1% sole blocker (hairline — likely capex cycle, not structural); P/E 16x cheap for quality; grade A; gate: FCF turning consistently positive; added 2026-08-12
    'SONACOMS.NS',    # Sona BLW Precision — precision differential gears + motor housings for EV + ICE vehicles; EV content per vehicle is 2-3x ICE for Sona's products; ~80% of EV differential assemblies in India; OM 16.6%, NM 14.2%, D/EV 0.009, GrossM 49.9%, RevG 52.4%; ROE/FCF 0.000 yfinance artifacts; 4/4 MA, RSI 83, +57.8% above 87w MA = very extended — quality real, entry timing wrong right now; gate: price correcting to 87w MA zone before entry makes sense; added 2026-08-08
    'METROBRAND.NS',  # Metro Brands — organized footwear retail (Metro, Mochi, Walkway + Crocs/Fitflop franchise in India); asset-light (company-owned stores, no franchise fee drag); premiumization of Indian footwear = structural tailwind; OM 18.0%, NM 13.8%, D/EV 0.057, GrossM 58.0%, RevG 14.7%; ROE/FCF 0.000 yfinance artifacts; 0/4 MA chart broken; gate: ROE/FCF data normalizing + weekly MA structure recovering; added 2026-08-08
    'AAVAS.NS',       # Aavas Financiers — affordable housing finance (₹10-50L ticket, self-employed/informal income customers in Tier 2-4 cities); the "missing middle" — too small for HDFC, not served by banks; OM 54.5%, NM 42.5%, GrossM 99.3% (NBFC reporting); D/EV 0.635 structural (NBFC borrow-to-lend model — same read as CHOLAFIN/BAJFINANCE, not deteriorating); ROE/FCF yfinance artifacts; 0/4 MA; gate: ROE data normalizing + MA recovery + D/E delevering toward 0.40 as AUM seasons; added 2026-08-08
    'BIOCON.NS',      # Biocon — India's largest biopharmaceutical company (Kiran Mazumdar-Shaw); Biocon Biologics subsidiary (~55% owned) has one of only 3-4 genuine end-to-end biosimilar platforms globally (trastuzumab, adalimumab, insulin glargine, bevacizumab, pegfilgrastim) — 20yr manufacturing moat in complex fermentation + sterile fill-finish; numbers look bad because of 2022 Viatris biosimilars commercial acquisition (~$3.3B, ~$1.2B cash/debt component) — acquisition debt is crushing NM and FCF, not the operating business; 4/4 MA, RSI 59.6 — technically intact; gate: D/E delevering toward 1.0x + NM crossing 5% sustained + FCF consistently positive as Viatris integration synergies compound; added WATCHLIST 2026-08-08
    ...
    'EMAMILTD.NS',    # Emami — ayurvedic/OTC consumer brands (Navratna cooling oil market leader, Boroplus antiseptic cream, Zandu pain balm, Fair & Handsome, HE deo); strong rural + semi-urban distribution; GrossM 67.9%, OM 17.6%, NM 19.1%, D/EV 1.0% (near-zero debt), RevG 14.9%; ROE yfinance gap (real ~30%); fwdPE 19.7x cheap for brand moat; 4/6 gates (ROE+FCF gaps only); grade A; gate: ROE data normalizing; added 2026-08-12
